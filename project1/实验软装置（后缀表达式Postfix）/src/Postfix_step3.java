	...
	void rest() throws IOException {
		...
		while (flag) {
			if (lookahead == '+') {
				...
				String temp = "There should be +/- between position" + position.toString()+" and position" + new Integer(position+1).toString();
				error_list.add(temp);
				state = false;
				term();
			} else if ((char)lookahead == ' ') {
				...
				state = false;
				position++;
				String temp = "There are "+ space_num.toString()+" extra blank space between position"+new Integer(position-space_num).toString() +" and position" + position.toString();
				position--;
				error_list.add(temp);
				rest();
			} else {
				match(lookahead);
				String temp = "The character at position"+position.toString()+" is invalid input, replace it with +/-";
				error_list.add(temp);
				state = false;
				term();
			}
	...
	void term() throws IOException {
		if (Character.isDigit((char)lookahead)) {
			...
			if ((char)lookahead == ' ') {
				...
				state = false;
				position++;
				String temp = "There are "+ space_num.toString()+" extra blank space between position"+new Integer(position-space_num-1).toString() +" and position" + position.toString();
				position--;
				error_list.add(temp);
				term();
			} else if ((char)lookahead == '+' || (char)lookahead == '-') {
				if (position == 0) {
					String temp = "The start of input should be integer";
					error_list.add(temp);
				} else {
					String temp = "There is missing a number between position"+ position.toString()+" and position"+new Integer(position+1).toString();
					error_list.add(temp);
				}
				state = false;
			} else if ((char)lookahead == '\n' || lookahead == 13) {
				position++;
				String temp = "There is missing a right operator at position"+position.toString();
				error_list.add(temp);
				state = false;
			} else {
				match(lookahead);
				String temp = "The character at position"+position.toString()+" is invalid input, replace it with 0~9";
				error_list.add(temp);
				state = false;
			}
